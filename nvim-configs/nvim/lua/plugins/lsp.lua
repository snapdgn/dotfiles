return {
    {
        ...
        branch = "v3.x",
        dependencies = {
            -- LSP Support (these are loaded with lsp-zero)
            { "williamboman/mason.nvim" },
            { "williamboman/mason-lspconfig.nvim" },
            { "neovim/nvim-lspconfig" },

            -- Autocompletion (these are loaded with lsp-zero)
            { "hrsh7th/nvim-cmp" },
            { "hrsh7th/cmp-buffer" },
            { "hrsh7th/cmp-path" },
            ...
            -- Snippets (these are loaded with lsp-zero)
            { "L3MON4D3/LuaSnip" },
            { "rafamadriz/friendly-snippets" },
        },
        -- Lazy-load on "BufReadPre" (before reading a buffer)
        -- This ensures LSP is ready when you open any file.
        -- Alternatively, use "FileType" for specific filetypes, e.g., "FileType lua,rust"
        event = "BufReadPre",
        config = function()
            ...
            lsp_zero.on_attach(function(_, bufnr)
                ...
                vim.keymap.set("n", "gI", function() vim.lsp.buf.type_definition() end,
                    { buffer = bufnr, remap = false, desc = "LSP Go to Type Definition" })
                vim.keymap.set("n", "gr", function() vim.lsp.buf.references() end,
                    { buffer = bufnr, remap = false, desc = "LSP Go to References" })
            ...
            -- This will now be called when lsp-zero loads
...
--formatting = lsp_zero.cmp_format({ details = false }),
--mapping = cmp.mapping.preset.insert({
--['<CR>'] = cmp.mapping.confirm({ select = true }), -- Use <CR> to confirm completion
